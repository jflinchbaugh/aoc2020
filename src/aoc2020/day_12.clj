...
(defn parse [input]
  (->>
    ...
    s/trim
    s/split-lines
    (map #(-> [(first %) (Integer/parseInt (s/join (rest %)))]))))

(def dirs-right {\N \E, \E \S, \S \W, \W \N})

(def dirs-left {\N \W, \W \S, \S \E, \E \N})
...
(defn next-pos [[[x y] heading] [i v]]
  (case i
    ...
    \E [[(+ x v) y] heading]
    \W [[(- x v) y] heading]
    \F (next-pos [[x y] heading] [heading v])
    \L [[x y] (new-heading heading dirs-left v)]
